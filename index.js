...
const { user: User, todoList: TodoList } = require("./models");
...
app.delete("/users/:userId", async (req, res, next) => {
  try {
    ...
    if (!toDelete) {
      ...
      res.json({ message: `user ${toDelete.id} was deleted` });
    }
  } catch (e) {
    next(e);
  }
...
// Create new todolist for user based on userId
...
// Delete a todolist
app.delete("/todolists/:listId", async (req, res, next) => {
  try {
    const listId = parseInt(req.params.listId);
    const toDelete = await TodoList.findByPk(listId);
    if (!toDelete) {
      ...
      const deleted = await toDelete.destroy();
      res.json({
        message: `The list: ${toDelete.name} with ID:${toDelete.id} was deleted`,
      });
    }
  } catch (e) {
    next(e);
